#!/bin/bash
OPENDISTRO_DN="/C=FR/ST=IDF/L=PARIS/O=EXAMPLE"
mkdir -p certs/{ca,os-dashboards,logstash,os01}
...
openssl x509 -req -extfile <(printf "subjectAltName=DNS:localhost,IP:127.0.0.1,DNS:os-dashboards") -in certs/os-dashboards/os-dashboards.csr -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out certs/os-dashboards/os-dashboards.pem
rm certs/os-dashboards/os-dashboards-temp.key certs/os-dashboards/os-dashboards.csr
openssl genrsa -out certs/logstash/logstash-temp.key 2048
openssl pkcs8 -inform PEM -outform PEM -in certs/logstash/logstash-temp.key -topk8 -nocrypt -v1 PBE-SHA1-3DES -out certs/logstash/logstash.key
openssl req -new -subj "$OPENDISTRO_DN/CN=logstash" -key certs/logstash/logstash.key -out certs/logstash/logstash.csr
openssl x509 -req -in certs/logstash/logstash.csr -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out certs/logstash/logstash.pem
rm certs/logstash/logstash-temp.key certs/logstash/logstash.csr
for NODE_NAME in "os01"; do
    ...
    openssl x509 -req -extfile <(printf "subjectAltName=DNS:localhost,IP:127.0.0.1,DNS:$NODE_NAME,DNS:opensearch") -in "certs/$NODE_NAME/$NODE_NAME.csr" -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out "certs/$NODE_NAME/$NODE_NAME.pem"
    rm "certs/$NODE_NAME/$NODE_NAME-temp.key" "certs/$NODE_NAME/$NODE_NAME.csr"
done
chown -R groupe1:groupe1 certs
